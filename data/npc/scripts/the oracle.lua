	...
	if(destination ~= nil and voc ~= nil and townid ~= nil) then
		if(getPlayerLevel(cid) < parameters.level) then
			npcHandler:say('CHILD! COME BACK WHEN YOU HAVE GROWN UP!')
                    ...
                    if (voc == 1) or (voc == 2) then  -- sorc/druid
					   ...
					   local bp = doPlayerAddItem(cid, 2000, 1) -- bag
						doAddContainerItem(bp, 2120, 1) -- rope
						doAddContainerItem(bp, 2554, 1) -- shovel
						doAddContainerItem(bp, 2789, 5) -- brown mushroom
						doAddContainerItem(bp, 2389, 5) -- spear

                    elseif (voc == 3) then  -- pally
                       doPlayerAddItem(cid, 2480) -- legion helmet
                       doPlayerAddItem(cid, 2484) -- studded armor
                       doPlayerAddItem(cid, 2530) -- cooper shield
                       doPlayerAddItem(cid, 2649) -- leather legs
                    ...
                    elseif (voc == 4) then  -- knight
					   ...
					   local bp = doPlayerAddItem(cid, 2000, 1) -- bag
						...
						doAddContainerItem(bp, 2554, 1) -- shovel
						doAddContainerItem(bp, 2789, 5) -- brown mushroom
						doAddContainerItem(bp, 2419, 1) -- sword
                    end
			npcHandler:say('SO BE IT, using command !items')
